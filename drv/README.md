...
`pwm1..pwm8` 是只读观测接口，采用标准 hwmon 的 `0..255` 标度上报。IT5570 文档中，常规 PWM 占空比定义为：

```text
DCRi / (CTR + 1)
```

但每个 PWM 通道的 **实际周期寄存器** 并不一定固定是 `CTR0`。驱动会在读取时，根据运行时的：
...
1. 观察所有标准节点与 raw 节点的基线值；
2. 结合平台固件、BIOS 设置或外部风扇控制手段改变实际风扇状态；
3. 同时观察对应的 `pwmN_dcr_raw` 以及全部 `fan*_tach_raw / fan*_input`；
4. 记录哪些风扇 tach 会随平台状态变化。
